...
int main (int argc, char **argv)
{
    ...
    if (verbose>0){
        ...
        printf("RunType=%s, k=%i, Verbose=%i\nnum_processes=%i, num_threads=%i\ndata_used=%i/41639\n",
                         str, k, verbose, 1, num_threads,  total_examples);
    }
                ...
                shared(num_data, total_time)
        {
            ...
            for (jj=0; jj<11; jj++){
                ...
                if (jj>0){
                    ...
                    iter_time = calctime(start_time, end_time);
    ...
    char buffer[128];
    // run_type _ num_process _ num_threads _ k _ data_used . txt
    snprintf(buffer, sizeof(char)*128, "RESULTS/%i_%d_%d_%d_%d.txt", 
        run_type, 1, num_threads, k, total_examples+bad_iter+1);
